...
def fetchstats(selected_user, df):
    ...
    mediaommitted = df[df['Message'].isin(['audio omitted','media omitted'])] 
...
def emojihelp(selecteduser, df):
    ...
    emojis = []
    for message in df['Message']:
        emojis.extend([c for c in message if c in emoji.EMOJI_DATA]) # Using emoji.EMOJI_DATA instead of emoji.UNICODE_EMOJI
